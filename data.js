...
const NutivaData = (() => {
  ...
  function metrics() { const current = new Date().toISOString().slice(0, 7); const sales = state.sales.filter(x => monthKey(x.date) === current); const expenses = state.expenses.filter(x => monthKey(x.date) === current); const productions = state.productions.filter(x => monthKey(x.date) === current); const revenue = sales.reduce((n, x) => n + x.price, 0); const cost = productions.reduce((n, x) => n + x.cost, 0); const outgoings = expenses.reduce((n, x) => n + x.amount, 0); const units = sales.reduce((n, x) => n + x.quantity, 0); return { revenue, cost, outgoings, profit: revenue - cost - outgoings, stock: state.products.reduce((n,p) => n + productStock(p.id), 0), avgCost: productions.reduce((n,x)=>n+x.quantity,0) ? cost / productions.reduce((n,x)=>n+x.quantity,0) : 0, avgPrice: units ? revenue / units : 0 }; }
  state = load();
